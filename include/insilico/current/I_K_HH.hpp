/*
 current/I_K_HH.hpp - Current flowing across neuronal membrane due to
...
#ifndef INCLUDED_I_K_HH_HPP
#define INCLUDED_I_K_HH_HPP
...
class I_K_HH {
 public:
  static void current(state_type &variables, state_type &dxdt, const double t, int index) {
    ...
    double alpha_n = (0.1-0.01 * (v+60))/(exp(1-0.1 * (v+60))-1.0);
    double beta_n = 0.125*exp(-(v+60) / 80.0);

    // ODE set
    dxdt[n_index]=(alpha_n*(1 - n)-beta_n * n);

    // Current
    engine::neuron_value(index, "I_K_HH", (gk * pow(n,4) * (v - ek)));
