/*
 current/I_Na_HH.hpp - Current flowing across neuronal membrane due to
...
#ifndef INCLUDED_I_NA_HH_HPP
#define INCLUDED_I_NA_HH_HPP
...
class I_Na_HH {
 public:
  static void current(state_type &variables, state_type &dxdt, const double t, int index) {
    ...
    double alpha_m = (2.5-0.1 * (v+60))/(exp(2.5-0.1 * (v+60))-1.0);
    double beta_m  = 4.0*exp(-(v+60) /18.0);
    double alpha_h = 0.07*exp(-(v+60) / 20.0);
    double beta_h  = 1.0/(exp(3-0.1 * (v+60))+1);
    ...
    // Current
    engine::neuron_value(index, "I_Na_HH", (gna * pow(m, 3) * h * (v - ena)));
