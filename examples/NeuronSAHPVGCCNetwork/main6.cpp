...
int main(int argc, char **argv) {
  ...
  configuration::observe_neuron(0,"il2");
  ...
  //configuration::observe_neuron(1,"fraction_of_open_channels");
   //configuration::observe_neuron(1,"il2");
    //configuration::observe_neuron(1,"tw");
    //configuration::observe_neuron(1,"s2");
  engine::generate_neuron<Neuron_sahp_vgcc>(1);
  ...
  while(tstop <=1000000.0){
	...
	deriv_type det(res); 
	tstop = engine::neuron_value(0, "tstop");
	double ts2 = engine::neuron_value(0, "ts2");
	
	double tw = engine::neuron_value(0, "tw");
  ...
  for(unsigned i=0 ; i < variables.size() ; ++i) {
		...
		tstop += dt;
		 insilico::engine::neuron_value(0, "dt", dt);
		  insilico::engine::neuron_value(0, "tstop", tstop);
		I_cav2::current(variables, det, tstop, 0);
		ts2 = engine::neuron_value(0, "ts2");
		//std::cout << ts2<<" "<<  "2"<< std::endl;
		  insilico::engine::neuron_value(0, "dt", dt);
		  insilico::engine::neuron_value(0, "tstop", tstop);
