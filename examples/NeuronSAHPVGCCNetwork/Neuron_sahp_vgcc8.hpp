...
class Neuron_sahp_vgcc : public Neuron {
 public:
  void ode_set(state_type &variables, state_type &dxdt,
    ...
    double ff = engine::neuron_value(index, "ff");
    ...
    double esyn_values = engine::get_pre_neuron_values(index, "esyn");
    double gsyn_values = engine::get_pre_neuron_values(index, "gsyn");
    double s_indices = engine::get_pre_neuron_indices(index, "s");
   // g1_indices = engine::get_pre_neuron_indices(index, "g1");
    //esyn_values = engine::get_pre_neuron_values(index, "esyn");
   std::cout << esyn_values<<std::endl;
   //std::cout << current;
    I_Syn = I_Syn + gsyn_values*variables*s_indices * (v - esyn_values);
    /*
    for(std::vector<int>::size_type iterator = 0; iterator < s_indices.size(); ++iterator) {
      I_Syn = I_Syn + gsyn_values*variables*s_indices * (v - esyn_values);
      //ff = (v - esyn_values[iterator]);
    }*/
 ...
 if (tstop> tq+7.0){
  if (fail == 1.0){
   ...
   /*if (tstop < 1000.0){
   gg = 0.0;}*/
    dxdt[v_index] = (gg*I_Ext) - I_Na - I_K - I_Leak -0.0*I_Syn -I_sahp- (I_cav2) ;
     /*if (index == 1){
   //std::cout << "Hi"<< " " <<std::endl ;
    I_cav2::current(variables, dxdt, t, index); */
  double tn = (gg*I_Ext) - I_Na - I_K - I_Leak -/*I_Syn -*/I_sahp- (I_cav2);
    ...
    insilico::engine::neuron_value(index, "fail", fail);
     ...
     insilico::engine::neuron_value(index, "ff", ff);
